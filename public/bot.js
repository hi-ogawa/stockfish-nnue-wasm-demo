...
class Api {
  ...
  async request(path, method, body) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(
        `fetch failure (${method}, ${url}, ${resp.status}, ${text})`
      );
    }
    return resp;
  }
  ...
  streamEvent() {
    return this.GET_NDJSON("/api/stream/event");
  }
  createChallenge(username, params) {
    return this.POST(`/api/challenge/${username}`, params);
  }
  ...
  streamGame(gameId) {
    return this.GET_NDJSON(`/api/bot/game/stream/${gameId}`);
  }
  move(gameId, move) {
...
class Bot {
  ...
  async start() {
    ...
    if (!ok) {
      throw new Error("Failed to verify bot account");
    }
    ...
    for await (const data of iterator) {
      ...
      if (data.type === "challenge") {
        ...
        const ok =
          ...
          0 <= timeControl.limit &&
          timeControl.limit <= 30 * 60 &&
          0 <= timeControl.increment &&
    ...
    for await (const data of iterator) {
      ...
      if (first_line) {
        ...
        if (data.type !== "gameFull") {
          throw new Error('Expected [type = "gameFull"]');
        }
        if (data.initialFen != "startpos") {
          throw new Error('Expected [initialFen = "startpos"]');
        }

        const { white, black } = data;
        if (this.bot_id !== white.id && this.bot_id !== black.id) {
          throw new Error(
            `Expected ${white.id} or ${black.id} = ${this.bot_id}`
          );
        }
      ...
      const bestmove = await this.engine.search(
        "startpos",
        moves,
        wtime,
        btime,
